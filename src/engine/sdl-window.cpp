...
{
  SDL_SetWindowSize(window_instance_.get(), screen_dimensions_.first, screen_dimensions_.second);
}

auto Engine::SDL::Window::getCurrentDimensions() const noexcept -> std::pair<std::int32_t, std::int32_t>
{
  return screen_dimensions_;
}

auto Engine::SDL::Window::createWindow() const noexcept -> std::expected<SDL_Window*, std::string> 
{
  const auto [width, height] = screen_dimensions_;

  const auto result {SDL_CreateWindow("Virtual Pet 2", width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE)};
  if(result == nullptr)
  {
    return std::unexpected(std::format("Failed to created SDL Window! Error: {}", SDL_GetError()));
  }
